...
(deftest hudson-maven-test
  (is (= (stevedore/do-script
          (directory/directory*
           "/usr/share/tomcat6/.m2" :group "hudson" :mode "g+w")
          (directory/directory*
           "/var/lib/hudson"
           :owner "root" :group "hudson" :mode "775")
          (remote-file/remote-file*
...
(deftest plugin-test
  (is (= (stevedore/do-script
          ...
          (user/user* "hudson" :action :manage :comment "hudson"))
         (test-resource-build [nil nil]
          (plugin :git)))))
