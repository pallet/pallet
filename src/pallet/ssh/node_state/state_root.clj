...
(defn- adjust-root
  [action-options ^String script-dir ^String path]
  (if (or (.startsWith path "/")
    ...
    (fragment
     (file ~(or script-dir
                (user-home (:username action-options)))
           ~path))))

(defn state-path*
  "Return a path under state-root for a given path."
  [action-options state-root script-dir path]
  (str state-root (adjust-root action-options script-dir path)))

(defn state-path
  "Return a path under state-root for a given path."
  [action-options state-root path]
  (state-path* action-options
...
(defrecord StateRootBackup [state-root]
  FileBackup
  (new-file-content
    [_ action-options path options]
    ;; create the state-root dir
    (let [state-path (state-path action-options state-root path)]
