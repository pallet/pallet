(ns pallet.ssh.execute
  ...
  (:require
   ...
   [pallet.script.lib :as lib :refer [chown env exit mkdir]]
        ...
        (cond
         ...
         :else (do
                 (when (transport/open? connection)
                   (release-connection ssh-connection connection))
                 r))
        (do
          (when (transport/open? connection)
            (release-connection ssh-connection connection))
    ...
    (with-context {:target (:server endpoint)}
      ...
      (with-connection ssh-connection node user [connection]
        (let [script (script-builder/build-script options script action)
              sudo-user (or (:sudo-user action) (user :sudo-user))
              tmpfile (ssh-mktemp
...
(defn- ssh-upload
  ...
  (logging/infof "Transferring file %s from local to %s" file remote-name)
...
(defn ssh-from-local
  ...
  (with-connection ssh-connection node user [connection]
    (let [{:keys [local-path remote-path remote-md5-path]} value]
      (logging/debugf "Remote local-path %s from %s" remote-md5-path local-path)
      (let [md5 (try
